...
def home(request):
    ...
    busnum = request.GET['busnum']
    direction = request.GET['direction']
    url = "http://realtime.portauthority.org/bustime/api/v2/getstops?key=AGqwpJtVsFgmULpgWHdH3vMdZ&format=json&rt=" + busnum + "&dir=" + direction  
    r = requests.get(url)
    data = r.json()
    stopdata = data['bustime-response']['stops']
    context['busnum'] = busnum
    context['direction'] = direction
...
def retrieve(request):
    ...
    stopname = "5th Avenue at Negley Ave"
    stid = 0
